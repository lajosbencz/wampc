...
export class Protocol {
    ...
    ping(payload: string | number): void {
        ...
        return this.write(payload, ProtocolMessageTypes.PING, this.setupPingTimeout.bind(this));
    }

    protected setupPingTimeout(): void {
        if (this._options.ping_timeout !== undefined && this._options.ping_timeout > 0) {
            this._ping_timeout = setTimeout(this.onPingTimeout.bind(this), this._options.ping_timeout);
        }
    }
    ...
    protected read(data: any): void {
        ...
        switch (this._status) {
            ...
            case ProtocolStatus.UNINITIATED:
                this._emitter.emit('error', new ProtocolError('Unexpected packet'));
                break;
    ...
    protected handleHandshake(int32: Buffer): ProtocolStatus {
        ...
        if (int32[0] !== ProtocolMagicByte) {
            const err = new ProtocolError(
                'Invalid magic byte. Expected 0x' + ProtocolMagicByte.toString(16) + ', got 0x' + int32[0].toString(16),
            );
        ...
        if ((int32[1] & 0x0f) === 0) {
            ...
            const err = new ProtocolError(
                'Peer failed handshake: ' + (ProtocolErrors[errCode] ?? '0x' + errCode.toString(16)),
            );
            this._emitter.emit('error', err);
            return this.close();
        }
    ...
    protected handleHeaderPacket(int32: Buffer): ProtocolStatus {
        ...
        switch (type) {
            ...
            default:
                this._emitter.emit('error', new ProtocolError('Invalid frame type: 0x' + type.toString(16)));
                return this.close();
        }
    }
    ...
    public removeListener(evt: string | symbol, handler: (...args: any[]) => void): void {
